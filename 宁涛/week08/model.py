...
class Attention(nn.Module):
    ...
    def forward(self, enc_output, dec_output):
        ...
        # dec_output: [batch_size, seq_len, hidden_dim * 2]
        # dec_output.permute(0, 2, 1) = [batch_size, hidden_dim * 2, seq_len]
        # a_t.shape = [batch_size, seq_len, seq_len]
        ...
        # a_t.shape = [batch_size, seq_len, seq_len]
        # a_t.permute(0, 2, 1) = [batch_size, seq_len, seq_len]
        # enc_output: [batch_size, seq_len, hidden_dim * 2]
        # c_t.shape = [batch_size, seq_len, hidden_dim * 2]
...
class Decoder(nn.Module):
    ...
    def forward(self, token_seq, hidden_state, enc_output):
        # hidden_state = [1, batch_size, hidden_dim * 2]
        # token_seq: [batch_size, seq_len]
        # embedded: [batch_size, seq_len, emb_dim]
        ...
        # attention运算
        # c_t = [batch_size, seq_len, hidden_dim * 2]
        c_t = self.atteniton(enc_output, dec_output)
        # [attention, dec_output]
        # dec_outputs: [batch_size, seq_len, hidden_dim * 2]
        # cat_output = [batch_size, seq_len, hidden_dim * 4]
