...
def create_sequences(data, sequence_length):
    ...
    for i in range(len(data) - sequence_length):
        ...
        y = data[i + sequence_length]  # 预测下一个时间步的值
        xs.append(x)
        ys.append(y)
...
if __name__ == '__main__':
    ...
    log_dir = './logs/homework2_RNN_1_day'
    ...
    y_train = all_y[:train_size].reshape(-1, 1)

    X_test = all_x[train_size:train_size+test_size].reshape(-1, input_sequence_length, 1)  # 添加通道维度
    y_test = all_y[train_size:train_size+test_size].reshape(-1, 1)

    # print("X_train shape:", X_train.shape)  # torch.Size([20000, 30, 1])
    # print("y_train shape:", y_train.shape)  # torch.Size([20000, 1])
    # print("X_test shape:", X_test.shape)    # torch.Size([5000, 30, 1])
    # print("y_test shape:", y_test.shape)    # torch.Size([5000, 1])
    ...
    # 定义模型
    model = RNN_Classifier(input_size=1, hidden_size=20, num_layers=2, outpoust_size=1)
    model.to(device)
