...
if __name__ == '__main__':
    mypath = 'D:/vsCodeProj/AiPremiumClass/王健钢/week8'
    enc_data,dec_data = read_data(mypath + 'cmn.txt')
    ...
    # 数据整体json数据集（json）
    with open(mypath +'encoder.json', 'w', encoding='utf-8') as f:
        json.dump(enc_data, f)  
    
    with open(mypath +'decoder.json', 'w', encoding='utf-8') as f:
        json.dump(dec_data, f)  

    import pickle
    with open(mypath +'vocab.bin','wb') as f:
