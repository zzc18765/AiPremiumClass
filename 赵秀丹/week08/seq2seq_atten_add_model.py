...
class Encoder(nn.Module):
    ...
    def forward(self, token_seq):
        ...
        return torch.add(hidden[0], hidden[1]), outputs
        # 返回最后一个时间步的隐状态（相加）
...
class Attention(nn.Module):
    ...
    def forward(self, enc_output, dec_output):
        # a_t = h_t @ h_s   enc_out_put.shape = 512  dec_output
...
class Decoder(nn.Module):
    def __init__(self, input_dim, emb_dim, hidden_dim, dropout):
        ...
        self.fc_dec=nn.Linear(hidden_dim,hidden_dim*2)
        # attention层
    ...
    def forward(self, token_seq, hidden_state, enc_output):
        ...
        dec_output=self.fc_dec(dec_output)
        # attention运算
        c_t = self.atteniton(enc_output, dec_output)
        # [attention, dec_output]
        # cat_output = torch.cat((c_t, dec_output), dim=-1)
        add_output = torch.add(c_t, dec_output)
        # 线性运算
        out = torch.tanh(self.atteniton_fc(add_output))
