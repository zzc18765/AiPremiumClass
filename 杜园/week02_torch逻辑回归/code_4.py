"""
    ...
    理解
        ...
        步骤：
            ...
            2、定义损失函数（原预测函数带入sigmoid函数求时间发生概率最大值（取反即求取损失函数最小值）
            3、定义梯度下降算法（）
"""
...
for i in range(epochs):
    # 计算预测值
    y_hat = model(X_train, theta, bias)
    # ① 计算损失函数值
...
x = X_test[idx]
y = y_test[idx]
y_hat = model(x, theta, bias)
print(f"特征值：{theta}, 截距：{bias}")
print(f"预测值：{y_hat}, 真实值：{y}")