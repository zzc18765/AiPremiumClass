...
def get_batch(split):
    ...
    x = torch.stack([data[i:i+block_size] for i in ix])       # 本身是list中的四个元组，然后转成矩阵
    y = torch.stack([data[i+1:i+block_size+1] for i in ix])   # 用于将多个张量沿着一个新的维度堆叠起来。尤其是在需要将它们组合成一个新的张量时
...
class Head(nn.Module):
    ...
    def forward(self, input_x):
        ...
        T = wei.shape[-1]                             
        tril = torch.tril(torch.ones(T,T, device=device))    # 设置 T，T大小的掩码矩阵，进行混淆 
        wei = wei.masked_fill(tril == 0, float('-inf'))
        wei = wei.softmax(dim=-1)                            # 转换为概率，-1表示行，相加为1
                                                             # 这里实际上做的就是self attention公式的还原
        out = wei @ v                                        #  B,T,T @ B,T,C = B,T,C 最后又还原回去了
        return out
