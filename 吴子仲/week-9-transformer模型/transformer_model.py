...
def create_mask(enc_input, dec_input):
    ...
    # enc_pad_mask, dec_pad_mask(AI生成优化)
    enc_pad_mask = enc_input == 0   # 假设 0 是 padding 的标记
    dec_pad_mask = dec_input == 0   # 假设 0 是 padding 的标记
    enc_pad_mask = enc_pad_mask.to(torch.float)
    dec_pad_mask = dec_pad_mask.to(torch.float)
...
if __name__ == "__main__":
    ...
    # 测试位置编码
    # pe = PositionalEncoding(emb_dim=512, dropout=0.1)
    # x = torch.randn(2, 10, 512)  # batch_size=2, seq_len=10, emb_dim=512
    # output = pe(x)
    # print(output.size())  # 应该是 (2, 10, 512)
    ...
    # 构建词典
    vocab = Vocabulary.build_from_data(chs)
    ...
    model = Seq2SeqTransformer(d_model, nhead, num_encoder_layers, num_decoder_layers,
                               dim_feedforward, dropout, enc_vocab_size, dec_vocab_size)
   ...
   # 定义损失函数和优化器
    ...
    for epoch in range(epochs):
        ...
        for enc_input, dec_input, labels in tpbar:
            ...
            # writer.add_scalar("Loss/train", total_loss/len(dataloader), epoch * len(dataloader) + tpbar.n)
