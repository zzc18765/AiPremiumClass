...
class Comments_Classifier(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, hidden_size, num_classes, rnn_type='LSTM'):
        super().__init__()
...
if __name__ == '__main__':
    ...
    def convert_data(batch_data):
        ...
        return commt, labels
    #--------------------------------------------------
    train_size = int(len(comments_data)*0.7)
    valid_size = int(len(comments_data)*0.2)
    
    # 训练集
    train_data = comments_data[:train_size]
    # 验证集
    validate_data = comments_data[train_size:train_size+valid_size]
    # 测试集
    test_data = comments_data[train_size+valid_size:]
 
    def build_rnn_model(rnn_type='LSTM'):
        model= Comments_Classifier(len(vocab), embedding_dim, hidden_size, num_classes, rnn_type='LSTM')
        model.to(device)
        # 损失函数
    ...
    for rnn_type in rnn_types:
        ...
        for epoch in range(num_epochs):
            ...
            for i, (cmt, lbl) in enumerate(dataloader):
                ...
                if (i+1) % 1000 == 0:
                    print(f'{rnn_type} Epoch [{epoch+1}/{num_epochs}], Step [{i+1}/{len(dataloader)}], Loss: {loss.item():.4f}')
                    writer.add_scalar(f'{rnn_type}/Loss', loss.item(), epoch*len(dataloader)+i)
            # 验证模型
            model.eval()
            with torch.no_grad():
                correct = 0
                ...
                print(f'{rnn_type} Accuracy : {100 * correct / total}%')
                writer.add_scalar(f'{rnn_type}/Accuracy', 100 * correct / total, epoch)

        # 保存模型
        torch.save(model.state_dict(), f'{rnn_type}comments_classifier.pth')
        # 模型词典
        torch.save(vocab, f'{rnn_type}comments_vocab.pth')
        ...
        # 加载词典
        vocab = torch.load(f'{rnn_type}comments_vocab.pth')
        # 测试模型
        ...
        model.load_state_dict(torch.load(f'{rnn_type}comments_classifier.pth'))
        model.to(device)
