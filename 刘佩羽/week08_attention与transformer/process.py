...
def mergeEncoderAndDecoderDocumment(encoder_data_file, decoder_data_file, file_path):
    """
    合并编码数据与解码数据
    """
    with open(encoder_data_file, encoding='utf-8') as f:
        enc_data = f.read().split('\n')

    with open(decoder_data_file, encoding='utf-8') as f:
    ...
    # 合并
    data = [enc + '\t' + dec for enc, dec in zip(enc_data, dec_data)]
...
if __name__ == '__main__':
    ...
    mergeEncoderAndDecoderDocumment(encoder_data_file='./couplet/train/in.txt' ,
                                    decoder_data_file='./couplet/train/out.txt' ,
                                    file_path='./trainMergeData.txt')
    mergeEncoderAndDecoderDocumment(encoder_data_file='./couplet/test/in.txt' ,
                                    decoder_data_file='./couplet/test/out.txt' ,
                                    file_path='./testMergeData.txt')

    # 训练数据读取
    enc_data, dec_data = read_data('./trainMergeData.txt')
    # # 测试数据读取
    # enc_data, dec_data = read_data('./testMergeData.txt')
