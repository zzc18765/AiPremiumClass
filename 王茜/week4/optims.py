...
if __name__ == '__main__':
    ...
    model0, model1, model2, model3 = nn.ModuleList(Net0() for _ in range(4))
    # 定义损失函数和优化器
    loss_fn = nn.CrossEntropyLoss().to(device)  # 交叉熵损失函数
    # torch.optim # 优化器库
    # 只需传入模型参数和学习率即可
    optimizer0 = torch.optim.SGD(model0.parameters(), lr=lr0)
    optimizer1 = torch.optim.RMSprop(model1.parameters(), lr=lr1)
    optimizer2 = torch.optim.Adam(model2.parameters(), lr=lr1)
    optimizer3 = torch.optim.AdamW(model3.parameters(), lr=lr1)

    loss_history_dict = {}
    # 训练
    loss0 = train_model(model0, train_batch, epochs, loss_fn, optimizer0)
    loss1 = train_model(model1, train_batch, epochs, loss_fn, optimizer1)
    loss2 = train_model(model2, train_batch, epochs, loss_fn, optimizer2)
    loss3 = train_model(model3, train_batch, epochs, loss_fn, optimizer3)
    # 绘制损失函数对比图像
    loss_history_dict['SGD'] = loss0
    loss_history_dict['RMSprop'] = loss1
    loss_history_dict['Adam'] = loss2
    loss_history_dict['AdamW'] = loss3
    loss_history_df = pd.DataFrame(loss_history_dict)
    plot_loss(loss_history_df)

    # 预测
    print(f'SGD: final loss: {loss0[-1]:.6f}')
    predict(model0, test_batch)
    print(f'RMSprop: final loss:{loss1[-1]:.6f}')
    predict(model1, test_batch)
    print(f'Adam: final loss:{loss2[-1]:.6f}')
    predict(model2, test_batch)
    print(f'AdamW: final loss:{loss3[-1]:.6f}')
    predict(model3, test_batch)
