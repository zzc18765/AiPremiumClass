...
def read_data(enc_data_file,dec_data_file):
    ...
    enc_data,dec_data = [],[]
    with open(dec_data_file,encoding='utf-8') as f:
        lines = f.read().split('\n')
        for line in lines:
            if line == '':
                continue
            dec = line.replace('，', '').replace('。', '').replace('？', '').replace('！', '')

            # 分词
            dec_tks = ['BOS'] + dec.split(' ') + ['EOS']
            # 保存           
            dec_data.append(dec_tks)    

    with open(enc_data_file,encoding='utf-8') as f:
        # 读取记录行
        lines = f.read().split('\n')
...
if __name__ == '__main__':
    enc_data_file='E:\\workspacepythonlearn\\week8\\archive\\couplet\\test\\in.txt'
    dec_data_file='E:\\workspacepythonlearn\\week8\\archive\\couplet\\test\\out.txt'
