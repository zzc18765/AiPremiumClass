...
class DialogSystem:
    def __init__(self):
        ...
        self.load_scenario("./邪王真眼/previous_class16/scenario/scenario-买衣服.json")

        self.slot_info = {}
        self.slot_template("./邪王真眼/previous_class16/scenario/slot_fitting_templet.xlsx")
    ...
    def get_intent(self, memory):
        ...
        for node_id in memory["available_nodes"]:
            ...
            if score > max_score:
                max_score = score
                hit_intent = node_id
        memory["last_hit_intent"] = memory.get("hit_intent", hit_intent)
    ...
    def nlg(self, memory):
        ...
        if memory["action"] == "answer":
            #直接回答
            answer = self.node_id_to_node_info[memory["hit_intent"]].get("response", None)
            if answer is None:
                return memory
...
if __name__ == '__main__':
    ...
    memory = {"available_nodes":["scenario-买衣服-node1", "scenario-买衣服-node5"]}
